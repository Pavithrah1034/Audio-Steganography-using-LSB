...
class LSBAudioStego():
    ...
    def encodeAudio(self, audioLocation, stringToEncode):
        # convert to array
    ...
    def decodeAudio(self, audioLocation) :
        # reconstruct original message by converting to binary array
        audioArray = self.convertToByteArray(audioLocation)
        decodedArray = [audioArray[i] & 1 for i in range(len(audioArray))]
        self.audio.close()
        return "".join( chr(int("".join(map(str, decodedArray[i:i + 8])), 2)) for i in
                        range(0, len(decodedArray), 8)).split("###")[0]
